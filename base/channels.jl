...
mutable struct Channel{T} <: AbstractChannel{T}
    ...
    data::Vector{T}
    @atomic length::Int                  # Length, tracked atomically
...
function _increment_channel_length(c, inc)
...
function put_buffered(c::Channel, v)
    ...
    try
        ...
        did_buffer || _increment_channel_length(c, -1)
        unlock(c)
    end
    return v
end
